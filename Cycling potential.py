...
# ---------------------------------------------------------------------------
LOCAL_RADIUS_METERS = 2000     
COMMUTER_RADIUS_METERS = 6000  
PROJECTED_CRS = 3857           
NUM_SIMULATION_TRIPS = 1000    
...
# ===========================================================================
# MAIN ENGINE TRY BLOCK (Indentation starts here)
# ===========================================================================
try:
    # -----------------------------------------------------------------------
    # 3. BOUNDARY & STREET NETWORK INGESTION
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # 4. SUPERCHARGED POI HARVESTING ENGINE
    # -----------------------------------------------------------------------
    OSM_LOCAL_TAGS = {
        "shop": True,              
        "amenity": True,           
    ...
    # -----------------------------------------------------------------------
    # 5. MONTE CARLO TRAFFIC PATH SIMULATOR
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # 7. GIS SHAPEFILE EXPORT (NETWORK + POIs)
    # -----------------------------------------------------------------------
    print("\n🗄️  PHASE 6: COMPILING ENGINE OUTPUTS & POIs INTO SHAPEFILES...")
    export_folder = "jaipur_cycling_gis_outputs"
    os.makedirs(export_folder, exist_ok=True)

    print("   -> Exporting Cycling Network Routes...")
    ...
    print("   -> Exporting Destination POIs...")
    if not local_pois.empty:
        local_export = local_pois.copy()
        local_export['category'] = 'local_errand'
        local_export.to_file(os.path.join(export_folder, "local_destinations.shp"), driver="ESRI Shapefile")
        
    if not commuter_pois.empty:
        commuter_export = commuter_pois.copy()
        commuter_export['category'] = 'commuter_anchor'
        commuter_export.to_file(os.path.join(export_folder, "commuter_anchors.shp"), driver="ESRI Shapefile")
